...
Requires:
    ...
    wrappers at it with :data:`TREX_ENV`, or with ``MOSAIC_TREX_CONDA_ENV`` /
    ``MOSAIC_TREX_BIN``; otherwise ``trex`` is found on ``$PATH``. TRex needs a
    display even headless -- run one persistent ``Xvfb`` and name it in the same
    placement (see :mod:`mosaic.tracking.trex.run`).

Usage
-----
>>> from pathlib import Path
>>> from mosaic.tracking.trex import (
...     TREX_ENV, TrexParams, run_trex_batch, run_trex_convert, run_trex_track,
... )
>>>
...
...     env=TREX_ENV.placed(conda_env="track", display=":99"),
... )
