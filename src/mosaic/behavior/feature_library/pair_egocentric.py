...
from typing import TYPE_CHECKING, Any, Iterable, List, Tuple, final
...
class PairEgocentricFeatures:
    ...
    def __init__(
        self,
        inputs: PairEgocentricFeatures.Inputs = Inputs(("tracks",)),
    ...
    def _map_anatomical_idx(self, param_name: str) -> int:
        ...
        """
        val = int(getattr(self.params, param_name))
        indices = self.params.pose.pose_indices
        if indices is None:
            return val
        try:
